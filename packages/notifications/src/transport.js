...
// ⚠️ THE TRANSPORT LIVES IN ITS OWN MODULE SO IT CAN BE REPLACED IN TESTS.
//
// It used to sit in index.js, and deliver.js imported `sendMail` from "./index.js". A test mocking
// the PACKAGE ("@hris/notifications") then had no effect: Vitest treats the bare specifier and the
// relative "./index.js" as different modules, so the delivery helper kept calling the real
// nodemailer while the stub sat unused — the suite passed while quietly trying to reach SMTP.
//
// Splitting it out gives every internal caller one module to import and tests one module to
// replace. The package's public surface is unchanged: index.js re-exports these.
