...
import { getEeoSummary, getErasureRequests, canManageErasure } from "@/lib/queries";
import { EraseCandidateForm, RefuseErasureForm } from "@/components/ErasureActions";

// The compliance screen — EEO aggregates and the erasure queue, for HR_ADMIN only.
//
...
export default async function CompliancePage() {
  ...
  const canManage = await canManageErasure();
  if (!canManage) notFound();

  const [eeo, requests] = await Promise.all([
    getEeoSummary(),
    getErasureRequests({ status: "PENDING" }),
  ]);
