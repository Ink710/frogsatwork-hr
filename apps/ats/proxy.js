...
export const config = {
  ...
  // /login, and /brand (public logo artwork — it must serve without a session, e.g. on the login
  // page, or the middleware 307s the image requests to /login and the Logo falls back to the glyph).
  matcher: ["/((?!api/auth|api/health|_next/static|_next/image|favicon.ico|login|brand).*)"],
};
