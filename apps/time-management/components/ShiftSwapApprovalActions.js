...
export function ShiftSwapApprovalActions({ swapId }) {
  const t = useT();
  const [state, action, pending] = useActionState(decideSwap.bind(null, swapId), undefined);
  ...
  return (
    <form action={action} className="flex flex-col items-stretch gap-2 sm:w-72">
      ...
      <div className="flex gap-2">
        ...
        <button type="submit" name="intent" value="deny" disabled={pending} className="flex-1 rounded-md bg-destructive px-3 py-1.5 text-xs font-medium text-primary-foreground hover:opacity-90 disabled:opacity-60">
          {t("approvals.deny")}
        </button>
      </div>
    </form>
