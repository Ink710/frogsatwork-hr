...
// only; the server re-checks ownership + that the target is in scope. Mirrors AssignmentEditor.
export function MeetingAssignmentEditor({ meetingId, assignees, candidates }) {
  const t = useT();
  const [addState, addAction, addPending] = useActionState(assignToMeeting.bind(null, meetingId), undefined);

  return (
    <div className="space-y-4">
      {assignees.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("meetings.noAssignees")}</p>
      ) : (
        <ul className="divide-y divide-border rounded-xl border border-border bg-card">
          {assignees.map((a) => (
            <li key={a.assignmentId} className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <form action={unassignFromMeeting.bind(null, meetingId, a.assignmentId)}>
                <button
                  type="submit"
                ...
                >
                  <X className="h-3.5 w-3.5" /> {t("meetings.remove")}
                </button>
              </form>
            </li>
          ))}
      ...
      {candidates.length > 0 ? (
        <form action={addAction} className="flex flex-wrap items-end gap-3">
          <div className="min-w-[16rem] flex-1">
            <label htmlFor="employeeId" className="block text-sm font-medium">{t("meetings.addAssignee")}</label>
            <select
              id="employeeId"
              name="employeeId"
              required
            ...
            >
              <option value="" disabled>{t("meetings.selectEmployee")}</option>
              {candidates.map((c) => (
        ...
        <p className="text-sm text-muted-foreground">{t("meetings.noCandidates")}</p>
      )}
