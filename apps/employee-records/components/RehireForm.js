...
export function RehireForm({ employeeId }) {
  ...
  return (
    <form action={formAction} className="mt-6 space-y-4">
      ...
      <div>
        <label className={labelCls} htmlFor="rehireDate">Rehire date</label>
        <input id="rehireDate" name="rehireDate" type="date" defaultValue={today} min={today} required className={fieldCls} />
        <p className="mt-1 text-xs text-zinc-400">Reopens an active record carrying forward their last title, type, and pay.</p>
      </div>
